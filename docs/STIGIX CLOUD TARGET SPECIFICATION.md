...
# peux tu m'expliquer toutes les options des targets stigix avec des exemples, c'est pour une doc md exhaustive et complete var __defProp = Object.defineProperty;
...
const clampedDelay = Math.min(Math.max(delay || 0, 0), 3e4);
if (clampedDelay > 0) {
...
const sizeBytes = typeof sizeParam === "string" \&\& sizeParam.endsWith("m") ? parseInt(sizeParam) * 1024 * 1024 : parseInt(sizeParam);

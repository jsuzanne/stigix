...
export const PrismaAppSyncModal: React.FC<PrismaAppSyncModalProps> = ({
    ...
    const loadPrismaStatus = async () => {
        ...
        try {
            ...
            setPrismaData({ success: false, error: err.message });
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const handleSyncSingleApp = async (appId: string) => {
        ...
        try {
            ...
            if (res.ok && data.success) {
                ...
                toast.error(data.error || 'Failed to sync application to Prisma SD-WAN');
            }
        } catch (err: any) {
    ...
    const handleSyncAll = async () => {
        ...
        try {
            ...
            if (res.ok && data.success) {
                ...
                toast.error(data.error || 'Failed to sync applications');
            }
        } catch (err: any) {
            toast.error(err.message || 'Network error');
        } finally {
    ...
    const handleCleanAll = async () => {
        ...
        try {
            ...
            if (res.ok && data.success) {
                ...
                toast.error(data.error || 'Failed to clean applications');
            }
        } catch (err: any) {
            toast.error(err.message || 'Network error');
        } finally {
    ...
    const prismaAppsList = prismaData?.apps || [];
                ...
                <div className="p-6 overflow-y-auto space-y-6 flex-1">
                                ...
                                <div className="text-xs font-bold text-text-primary flex items-center gap-2">
                                    ...
                                    {isLoading ? (
                                        ...
                                        <span className="text-emerald-600 dark:text-emerald-400 flex items-center gap-1 font-mono">
                                            ● Connected (TSG: {prismaData.tenant_id})
                                        </span>
                                    ) : (
                                        <span className="text-rose-500 font-medium">{prismaData?.error || 'Unavailable / Check Credentials'}</span>
                                    )}
                                </div>
                                <div className="text-[11px] text-text-muted mt-0.5">
                                    Total Tenant Apps: <strong className="text-text-secondary">{prismaData?.total_apps ?? '—'}</strong> | Managed by Stigix: <strong className="text-blue-500">{prismaData?.stigix_apps_count ?? '—'}</strong>
                                </div>
                            </div>
                        </div>
                    ...
                    <div className="space-y-3">
                        <div className="flex items-center justify-between text-xs font-bold text-text-muted uppercase tracking-wider">
                            <span>Stigix Custom TCP Apps ({applications.length})</span>
                        ...
                        <div className="border border-border rounded-2xl overflow-hidden divide-y divide-border">
                            {applications.length === 0 ? (
                                ...
                                applications.map(app => {
                                    ...
                                    const expectedPrismaName = `STX_${app.name.replace(/[^a-zA-Z0-9_\-]/g, '_')}`;
                                    const matchedPrismaApp = prismaAppsList.find(pa => 
                                        pa.name.toLowerCase() === expectedPrismaName.toLowerCase() ||
                                        pa.name.toLowerCase() === app.name.toLowerCase() ||
                                        pa.tcp_ports?.includes(Number(port))
                                    );
